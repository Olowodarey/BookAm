...
import { coordinatorApi, setToken } from "@/lib/dashboard/api";
...
export default function CoordinatorLoginPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { accessToken, user } = await coordinatorApi.login(phone, password);
      if (user.role !== "COORDINATOR") {
        setError(
          "This account is not a coordinator yet. Apply as a collector first.",
        );
        return;
      }
    ...
    <div className="flex min-h-dvh items-center justify-center bg-paper px-4">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-4 text-center text-xs text-muted">
          Your circle records live here — the money still moves hand to hand,
          exactly as before.
        </p>
      </div>
    </div>
