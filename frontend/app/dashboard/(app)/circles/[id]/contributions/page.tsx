...
export default function ContributionsPage() {
  ...
  // Shared cells so the desktop table and the mobile card list stay in step.
  const receiptButton = (c: ContributionInfo) =>
    c.receiptFileUrl ? (
      <button
  ...
  const statusBlock = (c: ContributionInfo) => (
    <>
      ...
      {c.status === "PAID" && c.reviewedByName ? (
        <p className="mt-1 text-xs text-muted">by {c.reviewedByName}</p>
      ) : null}
    </>
  );

  const actions = (c: ContributionInfo) => (
      ...
      {c.status !== "PAID" ? (
        <>
          <ReceiptFileButton
            ...
            onFile={(file) =>
              void run(c.id, () =>
                coordinatorApi.uploadContributionReceipt(circle.id, c.id, file),
              )
            }
          />
          <Button
            ...
            onClick={() =>
              void run(c.id, () =>
                coordinatorApi.verifyContribution(circle.id, c.id),
              )
            }
            aria-label={
          ...
          >
            {c.status === "PENDING_REVIEW" ? "Verify · mark paid" : "Mark paid"}
          </Button>
        </>
      ) : null}
      {c.status === "PENDING_REVIEW" ? (
  ...
  return (
    <div>
      ...
      {cycle.contributions.length === 0 ? (
        ...
        <>
          {/* Mobile: one card per member so nothing needs sideways scrolling. */}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-start gap-3">
                    <span className="flex h-9 w-9 shrink-0 flex-col items-center justify-center rounded-lg border border-green/25 bg-green/10 font-mono text-green">
                  ...
                  <div className="shrink-0 text-right">{statusBlock(c)}</div>
                </div>
          ...
          {/* Desktop: the full table. Scrolls within its own box if narrow. */}
              ...
              <tbody>
                {cycle.contributions.map((c) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">{statusBlock(c)}</td>
                    <td className="px-5 py-3.5">{receiptButton(c)}</td>
                    <td className="px-5 py-3.5">{actions(c)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
